  ...
  cart.forEach(item => {
    ...
    tr.innerHTML = `
      ...
      <td style="padding: 15px; display:flex; gap:12px; align-items:center;">
        ${item.image ? `<img src="${item.image}" alt="${escapeHtml(item.name)}" style="width:80px;height:80px;object-fit:cover;border-radius:6px;" loading="lazy" width="80" height="80">` : ''}
...
document.addEventListener('click', function (e) {
  ...
  // Check for a quantity input on the page (for product detail pages)
...
if (placeOrderBtn) {
  placeOrderBtn.addEventListener('click', () => {
    ...
    const order = {
      ...
      orderId: `FF-${Date.now()}`, // Generate a unique order ID
      userId: loggedInUser.email, // Link order to the user
      status: 'Processing'
    };
    ...
    localStorage.setItem('fftraders_orders', JSON.stringify(allOrders));

    console.log('Order placed and saved:', order);

    showCustomAlert(
      '✓ Order Placed Successfully!',
      `Your Order ID is ${order.orderId}. We'll contact you at: ${phone}`,
      '✓',
      () => {
